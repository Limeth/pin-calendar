...
async function AccountGetSecretKey(account: Account): Promise<CryptoKey> {
  return await crypto.subtle.importKey(
    'raw',
    account.secretKeyData,
    SECRET_KEY_ALGORITHM,
    SECRET_KEY_EXTRACTABLE,
    SECRET_KEY_USAGES,
  );
}
...
async function AccountGetId(
  key: Exclude<keyof Account, 'secretKeyData'>,
  account: Account,
): Promise<Id> {
...
function fromHexString(hexString: string): Uint8Array {
  ...
  if (match !== null) return Uint8Array.from(match.map((byte) => parseInt(byte, 16)));
