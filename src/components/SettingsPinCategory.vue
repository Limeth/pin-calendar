...
import { ref } from 'vue';
import { PinCatalogGetPinsInCategory, PinCatalogGetSubcategoriesInCategory, type PinCatalog, type PinCategoryTypeOf, type PinTypeOf } from '../pins';
          ...
          <ul class="collapse-content flex flex-col !py-0">
            <li v-for="pin of R.filter((pin) => !!pin.value.archived, PinCatalogGetPinsInCategory(pinCatalog, pinCategory.id))" :key="pin.id.key"
              ...
              <template v-if="!predecessorArchived && !pinCategory.value.archived">
                ...
                <button @click="emit('event', { kind: 'pinDelete', pin })"
                  class="btn btn-sm max-sm:btn-square btn-error">
                  <div v-html="feather.icons['trash-2'].toSvg()" />
                  <div class="max-sm:hidden">Delete</div>
                </button>
