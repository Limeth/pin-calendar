  ...
  <div class="drawer drawer-end">
    ...
    <div class="drawer-content">
      <div class="flex justify-center">
        <div class="flex flex-col bg-base-100 items-center lg:w-[64rem] lg:my-8 max-lg:w-full dark:bg-slate-500">
          ...
          <div class="w-full">
            <div class="grid w-full bg-slate-300 gap-[1px] lg:px-[1px] py-[1px] lg:grid-cols-7 lg:shadow-xl">
              <div v-for="day of dayList.slice(0, 7)" :key="PinCalendarDayToKey(day.date)"
                class="p-1 text-lg text-center max-lg:hidden">
                {{ day.date.toLocaleString(undefined, { weekday: 'short' }) }}
              </div>
              <div v-for="day of dayList" :key="PinCalendarDayToKey(day.date)" @click="clickDay(day)"
                ...
                <div class="flex flex-wrap gap-1 justify-start">
                  <template
                    v-for="pin of PinCalendarGetPinsOnDay(getPinCalendar().value, getPinCatalog().value, day.date)"
                    :key="pin.id">
                    <PinIcon :pin="pin" />
                  </template>
                </div>
    ...
    <div class="drawer-side w-full z-10" v-if="pinDay !== null">
      ...
      <div
        ...
        <div class="text-2xl mb-4 text-center">{{ daySelected?.date.toLocaleString(undefined, {
          day: "numeric", month:
            "long"
        }) }}</div>
        <!-- <ul class="menu">
