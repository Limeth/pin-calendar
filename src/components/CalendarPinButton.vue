...
import { PinDayHasPin, type PinDay } from '@/pins/pinCalendarDay';
import { type Pin, type PinCatalog, type PinTypeOf } from '../pins/pinCategory';
...
<template>
  <li
    class="pin-btn flex flex-row items-center gap-1 px-4 py-1 cursor-pointer"
    :class="{
...
.pin-btn-even-depth:nth-child(odd):hover:not(.pin-btn-active) {
  background-color: color-mix(
    in oklab,
    oklch(var(--b2)),
    v-bind('pin.value.backgroundColor') 20%
  ) !important;
}
