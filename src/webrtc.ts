...
export class WebRtcNetworkAdapter extends NetworkAdapter {
    ...
    this.peer.on('open', (id) => {
      ...
      for (const remotePeer of this.options.clientSettings.value.remotePeers) {
        ...
        dataConection.once('open', () => {
          this.onConnectionEstablished(dataConection);
        })
      }
    });

    this.peer.on('connection', (dataConnection) => {
      this.onIncomingConnectionRequest(dataConnection);
    });
  }
  ...
  getPeerJsPeerId(automergePeerId: PeerId): string | undefined {
    // TODO
    return undefined;
  }

  onIncomingConnectionRequest(dataConnection: DataConnection) {
    this.onConnectionEstablished(dataConnection);
  }

  onConnectionEstablished(dataConnection: DataConnection) {
    console.log("Connection established: ", dataConnection);
    ...
    const connectedPeer: ConnectedPeer = {
      ...
      // automergePeerId,
      // automergePeerMetadata: dataConnection.metadata,
    };
    ...
    const onOpen = () => {
      ...
      console.log(`Adding opened peer peerJsPeerId:${dataConnection.peer}`);
      ClientSettingsAddPeer(this.options.clientSettings.value, dataConnection.peer);
